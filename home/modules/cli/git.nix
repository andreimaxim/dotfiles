{ config, pkgs, ... }:
let
  signingKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBLsCFcEHXqWkgD65hJ9y4gARfqWT0FYyVOLhgUxl45c";
  onePasswordSigner = pkgs.writeShellScript "onepassword-ssh-sign" ''
    if [[ " $* " != *" -Y sign "* ]]; then
      exec ssh-keygen "$@"
    fi
    ...
    exit 127
  '';
  onePasswordSsh = pkgs.writeShellScript "onepassword-ssh" ''
...
in
{
  xdg.configFile."git/allowed_signers".text = ''
    andrei@andreimaxim.ro ${signingKey}
  '';
  ...
  programs.git = {
    ...
    settings = {
      ...
      pull.rebase = true;
      core.sshCommand = "${onePasswordSsh}";
      ...
      alias.diffs = "-c delta.side-by-side=true diff";
